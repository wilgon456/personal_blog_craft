...
- 영문 slug를 직접 입력하는 것을 기본 규칙으로 사용
- 단, slug 입력 없이 업로드된 경우에는 제목 기반으로 영문 slug를 자동 생성

확정된 규칙:

- slug는 영문만 허용
- slug는 직접 입력이 기본
- slug가 비어 있으면 제목을 기반으로 영문 slug를 자동 생성
...
- 태그 노출명은 자유롭게 쓰되 URL용 값은 정규화하는 방향

확정된 규칙:

- 태그는 한글과 영문을 혼용해서 사용 가능
- 화면 표시용 태그명은 원문 그대로 사용
- 태그 URL이나 내부 키가 필요할 경우 별도 정규화 규칙을 적용
